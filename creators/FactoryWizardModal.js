...
var FactoryWizardModal = class extends EntityWizardBase {
    ...
    async loadConfig() {
        try {
            ...
            console.log('DEBUG: FactoryWizardModal - Config loaded. climates:', this.config.climates);
            console.log('DEBUG: FactoryWizardModal - Data initialized. climate:', this.data.climate);
        } catch (e) {
    ...
    renderNavFlex(contentEl) {
        ...
        if (this.step < 8) {
            ...
            const finishBtn = right.createEl('button', { text: '✓ Создать завод' });
